...
for i in range(1):
    ...
    if True:
        parameter_ranges = {'lambda1': np.linspace(0.01, 0.3, 4),
                            'lambda2': np.linspace(0.01, 0.3, 4)}
        # parameter_ranges = {'lambda1': np.linspace(0.05, 0.45, 5),
        #                     'lambda2': np.linspace(0.01, 0.25, 7)}

        # B_heter = grid_search_hyperparameters_v1(parameter_ranges, X, delta, R, tree_structure, rho=1, eta=0.2,
        #                                          method='heter')
        B_proposed = grid_search_hyperparameters_v1(parameter_ranges, X, delta, R, tree_structure, rho=1, eta=0.2,
        ...
        B_proposed = ADMM_optimize(X, delta, R, lambda1=0.3, lambda2=0.1, rho=1, eta=0.1, B_init=B_notree, tree_structure=tree_structure)
