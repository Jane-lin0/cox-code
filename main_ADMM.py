...
N = sum(N_class)
X, delta, R = generate_simulated_data(G, N_class, p)
...
lambda2 = np.concatenate([np.zeros(G), np.ones(K-G) * 0.1])   # vector(K - G,) lambda2_u, 第 1,...,G 个节点是叶节点，不需要组惩戒，前 G 个值设为0


def ADMM_optimize(X, delta, R, lambda1, lambda2, rho=1, eta=1):
    # 初始化变量
    B1 = np.ones((G, p))
    B2 = np.ones((G, p))
    B3 = np.ones((G, p))
    ...
    for m in range(M):
        ...
        for u in internal_nodes(tree):
            ...
            updated_gamma_children = group_soft_threshold(Gamma1_child - W1_child, lambda2 / rho)    # # lambda2_u, lambda2[u-1]
        ...
        B3_old = B3.copy()
        for j in range(p):
            B3[:, j] = group_soft_threshold(B1[:, j] - U2[:, j], lambda1 / rho)    # lambda1_j, lambda1[j]
